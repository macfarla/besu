...
- Add `eth_getTransactionBySenderAndNonce` JSON-RPC method to look up a transaction by sender address and nonce (pending or mined). Requires `--tx-sender-nonce-index-enabled` to index historical mined transactions; disabled by default. [#10284](https://github.com/besu-eth/besu/issues/10284)
