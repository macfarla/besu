...
public class SlowBlockTracer implements BlockAwareOperationTracer {
  ...
  /**
   * Finds existing ExecutionMetricsTracer in the delegate or creates a new one. This optimizes
   * performance by reusing existing metrics collection when available.
   *
   * @return ExecutionMetricsTracer instance
   */
  private ExecutionMetricsTracer findOrCreateExecutionMetricsTracer() {
    if (delegate instanceof ExecutionMetricsTracer) {
      return (ExecutionMetricsTracer) delegate;
    } else if (delegate instanceof TracerAggregator) {
      final Optional<ExecutionMetricsTracer> existingTracer =
          ((TracerAggregator) delegate).findTracer(ExecutionMetricsTracer.class);
      if (existingTracer.isPresent()) {
        return existingTracer.get();
      }
    }
    // Create new ExecutionMetricsTracer if none found
    return new ExecutionMetricsTracer();
  }
  ...
  public void traceStartBlock(
    ...
    // Try to reuse existing ExecutionMetricsTracer from delegate, or create new one
    metricsTracer = findOrCreateExecutionMetricsTracer();
  }
