...
/**
 * Converts a {@link SyncTransactionReceipt} (holding raw wire-format bytes) into a canonical
 * {@link TransactionReceipt} that can be stored and later decoded by {@link
 * TransactionReceiptDecoder}.
 *
 ...
 * (eth/69/70, no bloom filter) encode log topics as raw bytes32 on the wire but the storage
 * decoder expects trimmed [leadingZeros, shortData] lists, causing an RLPException.
 */
public class SyncTransactionReceiptConverter {
   ...
   * isFormattedForRootCalculation=true}), the raw bytes are passed directly to {@link
   * TransactionReceiptDecoder} which can handle them as-is.
   *
  ...
  public static TransactionReceipt toTransactionReceipt(final SyncTransactionReceipt receipt) {
    if (receipt.isFormattedForRootCalculation()) {
      ...
      return TransactionReceiptDecoder.readFrom(RLP.input(receipt.getRlpBytes()), true);
    }
    ...
    SyncTransactionReceipt decoded = receipt.getLogs() != null ? receipt : DECODER.decode(receipt.getRlpBytes());

    if (decoded.isFormattedForRootCalculation()) {
      // Re-decoded result still has a bloom filter; use TransactionReceiptDecoder.
      return TransactionReceiptDecoder.readFrom(RLP.input(decoded.getRlpBytes()), true);
    }
